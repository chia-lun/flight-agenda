...
class VertexNames{ 
	...
	VertexNames(){
		VertexNames[0]="TPE";//Taipei
		VertexNames[1]="MKE";//Milwaukee, Wisconsin
		VertexNames[2]="BOS";//Boston 
		VertexNames[3]="JFK";//New York
		VertexNames[4]="PDX";//Portland
		VertexNames[5]="BWI";//Baltimore/Washington
		VertexNames[6]="PVG";//Shanghai, China
		VertexNames[7]="KIN";//Kingston, Jamaica
	}

	//Used during input, this function returns the corresponding airport index given the string of airport code
	int getAirportasIndex(String DepAirpt) {
		int i=0;

		//checks if strings are equal. Case insensitive
		while(VertexNames[i].equalsIgnoreCase(DepAirpt)==false){
			i++;
		}
		...
		for(i=0;i<tot_nodes;i++){
			for(j=0;j<tot_nodes;j++){
				if(i==j)
					...
					//1440 minutes in 24 hours so say 1441 is infinity. Initialize all other times as infinite (unreachable). 
